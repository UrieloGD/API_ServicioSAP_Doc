...
> 🗑️ **Descartado el 31 ago: `recommender/setRecommenderList`.** Mismo criterio — pierde su identificador y los posteriores se reindexan una posición. La serie vigente es **E-01 a E-48** sin huecos.
...
|---|---|---|---|---:|---|
| E-45 | `credit/SolicitudMercancia` | `partner/client/{clientId}` | `ServicioAndroid` | 1.0 | — |
| E-46 | `credit/codigoPromocion` | SuccessFactors + BP05 | **SIGMAVI** `VentaCupon` | 1.5 | — |
| E-47 | `credit/getPlazos` | TZ01 | **SIGMAVI** `CondicionesCredVtaLinea` | 1.5 | — |
| E-48 | `customerService/obtenerTipoGarantia` ⏳ | `Art` — maestro de materiales | **SIGMAVI** tabla nueva | 1.5 | — |

> **E-45** parte en dos el `INSERT ... SELECT` único del legado: leer el Business Partner en C# y luego insertar. Requiere el **helper de conversión de cuenta `C%` → BP**, que no existe y lo van a necesitar E-46, E-47 y varios mixtos.

> ⛔ **E-46 depende del wrapper de SuccessFactors** que construye Dev 1. El de TZ01 que usa E-47 ya existe.

> 🔒 **E-48 está bloqueado.** La tabla la alimenta PCP y **Miguel Marín** debe entregar la estructura. Al recibirla, verificar que conserve `TipoGarantia`, `Marca`, `Telefono`, `Proveedor`, `Linea`.
...
|---|---|---|---|
| 1 | **Estructura de garantías (Miguel Marín)** | E-48 no arranca | Solicitar antes del 1 de septiembre |
