...
Lista de control de las **76 partidas** (4 habilitadores + 72 endpoints) del plan de migración. Se va marcando aquí conforme se completa cada una. Alcance: todo lo que **no es Intelisis** (ServicioAndroid, SQLite, SIGMAVI, DMZ/SMB); los mixtos (Intelisis + otros) quedan documentados pero pendientes de decisión de arquitectura. La única pieza que toca SAP directamente (E-48) se deja preparada para que el equipo de SAP la conecte.
...
En la práctica esto recae sobre los mixtos `M-11`…`M-08` y sobre las partidas que hoy cruzan a Intelisis (`E-48`, `E-49`, `E-50`), que ya estaban fuera de la ruta principal.
...
- [ ] **E-09** `customerService/obtenerQuejas` — ServicioAndroid. **90 %**: escrito el 21 ago en `Methods\CustomerService\CustomerServiceMethods.cs`, compila en 0 errores, cutover commiteado y subido el 26 ago (`c695b2e` en APIMagentoDMZ). Los 5 casos verificados el 23 ago contra servicios reales; ficha lista. Falta desplegar.
- [ ] **E-10** `customerService/bbvaKeyAdvanced` — SOAP `WSeCommerceMX`. **90 %**: escrito el 21 ago, misma clase, cutover commiteado y subido el 26 ago (mismo commit que E-09). No toca base: es una llamada SOAP a `MULTIPAGOS_APIKEY_URL`, ya portada al `Web.config` junto con `CODIGO_ENT`. Falta probar y ficha. ✅ Verbo en paridad: la DMZ lo llama con `curl.GetSAP(...)`, entregado por Dev 1 el 21 ago.
...
> **Código commiteado y subido el 26 ago**: `4315c50` en `dbAndroid` de ServicioSAP, con las cuatro partidas, sus modelos y las cuatro claves nuevas del `Web.config`. El cutover de E-13 va en `e403065` de APIMagentoDMZ. Los porcentajes no se mueven por esto: commitear no es una casilla de la rúbrica, y lo que falta en cada partida sigue siendo lo mismo que el 25 ago.
...
- [ ] **E-13** `customer/cashCustomerReport` — filesystem + SMB. **80 %**: escrito el 25 ago; validación y escritura local verificadas, cutover commiteado y subido el 26 ago (`e403065`). Fuente real: `CustomerMethods.CreateCashReport` en APIMagento, ya usa `Impersonation` (H-02) para escribir en `\\172.16.200.2\mavica\ecom\BaseWhatsapp\STAGE\`. **Se puede escribir ya**: la clase `Impersonation` está lista y el llamador le pasa las credenciales (`SMB_IMPERSONATION_*`, orden `usuario, dominio, password`). La copia al share no es verificable desde desarrollo; se valida en QA junto con H-02.
...
- [ ] **E-15** `order/GetPickUpCode` — `SELECT ClaveVenta FROM TrWDM0285_CteRecoge`, una sola tabla, que se crea en SIGMAVI. Verificado el 12 ago: no lee nada de SAP.
...
- [ ] **E-48** `credit/SolicitudMercancia` — lee el Business Partner de SAP e inserta en `CRED_SOLICITUD_WEB_DATOS_TEMP` de `ServicioAndroid`. Requiere el helper de conversión de cuenta `C%` → BP.
- [ ] **E-49** `credit/codigoPromocion` — tabla `VentaCupon` en SIGMAVI. **Ya construido** como `HandlePromoCode`; falta alinear el nombre de la tabla, que hoy es `VentasCupones`.
- [ ] **E-50** `credit/getPlazos` — tabla `CondicionesCredVtaLinea` en SIGMAVI + condiciones contra TZ01.
- [ ] **E-51** `customerService/obtenerTipoGarantia` — tabla `DM0415` en SIGMAVI, poblada exportando desde Intelisis, + artículo contra DM01. Estructura pendiente de **Valentin y Humberto**.
...
- [ ] Estructura de `DM0415`, garantías — **Valentin y Humberto** (corregido el 12 ago) — bloquea E-51
...
**10 / 37** partidas marcadas como completadas (H-01, H-02, H-03, H-04, E-01, E-02, E-03, E-04, E-07, E-08). `[x]` aquí significa **desarrollo terminado**; varias tienen validaciones diferidas anotadas en su línea: H-02 y H-04 esperan QA, E-01 espera el canal de SMS, y las cuatro partidas de endpoints esperan el despliegue del cutover.

El contador solo cuenta partidas cerradas, así que esconde el trabajo a medias. El avance ponderado real es **49,0 %** sobre 34 partidas; el desglose por endpoint, con el criterio de cálculo y el estado de pruebas de cada uno, está en [[ESTADO_PRUEBAS_Y_AVANCE]].
...
La Ola 6 quedó escrita y probada el 25 ago, y **commiteada y subida el 26 ago**. Cierra en un rango amplio: E-11 al **90 %**, E-12 y E-13 al **80 %**, E-14 al **55 %**. Ninguna avanza más sin resolver dos cosas que no dependen del código — un id de cliente de Magento para probar la escritura de E-12, y H-02 para E-13 y E-14.

Todos los cutovers de las olas 1 a 6 están **commiteados y subidos** a `dbAndroid` de APIMagentoDMZ, pero **ninguno desplegado**: en producción el tráfico sigue yendo al legado.
